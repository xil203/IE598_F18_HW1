...
#df = StandardScaler().fit_transform(df[['CRIM', 'ZN', 'INDUS', 'CHAS','NOX', 'RM', 'AGE', 'DIS', 'RAD','TAX', 'PTRATIO', 'B', 'LSTAT', 'MEDV']])

#df[['CRIM', 'ZN', 'INDUS', 'CHAS','NOX', 'RM', 'AGE', 'DIS', 'RAD','TAX', 'PTRATIO', 'B', 'LSTAT', 'MEDV']] = StandardScaler().fit_transform(df1[['CRIM', 'ZN', 'INDUS', 'CHAS','NOX', 'RM', 'AGE', 'DIS', 'RAD','TAX', 'PTRATIO', 'B', 'LSTAT', 'MEDV']])
# from sklearn.preprocessing import StandardScaler
# df1 = StandardScaler().fit_transform(df[['MEDV']])
# df1
scaler = StandardScaler().fit(df)
df1 = scaler.transform(df)
df1
df2 = pd.DataFrame(df1, columns = ['CRIM', 'ZN', 'INDUS', 'CHAS','NOX', 'RM', 'AGE', 'DIS', 'RAD','TAX', 'PTRATIO', 'B', 'LSTAT', 'MEDV'])
df2
...
print('MSE train: %.3f, test: %.3f' % (mean_squared_error(y_train, y_train_pred), mean_squared_error(y_test, y_test_pred)))
print('R^2 train: %.3f, test: %.3f' %(r2_score(y_train, y_train_pred),r2_score(y_test, y_test_pred)))
print('Intercept: %.3f' %  enet.intercept_)
print('Slope: %.3f'  %  enet.coef_[0])
print('Intercept: %.3f' %  enet.intercept_)
print('Slope: %.3f'  %  enet.coef_[0])
...
y = df2['MEDV'].values 
X_train, X_test, y_train, y_test = train_test_split( X, y, test_size=0.3, random_state=0) 
enet = ElasticNet(alpha = 1,l1_ratio = 0) 
enet.fit(X_train, y_train)
y_train_pred = enet.predict(X_train)
y_test_pred = enet.predict(X_test)
plt.scatter(y_train_pred, y_train_pred - y_train,c='blue', marker='o', label='Training data') 
